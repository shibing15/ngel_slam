...
class Renderer:
    ...
    def raytrace(self, rays, num_samples, grid, decoders, lod_idx = None, test = False):
        ...
        if test:
            occ_unc = torch.ones(N, 1, device=rays.origins.device) * 0.25
    ...
    def get_hit(self, rays, gt_color, gt_depth, grid, decoders, test = False, dmax = 0):
        ret = self.raytrace(rays, self.samples_per_vox, grid, decoders, test = test)
    ...
    def query_occ(self, coords, grid, embedder, occ_decoder):
        shape = coords.shape
        lod_idx = grid.num_lods - 1
        result = grid.blas.query(coords.reshape(-1, 3), grid.active_lods[0])#, with_parents=True)
        ...
        occs[mask] = -100 #torch.abs(sdfs[mask])
