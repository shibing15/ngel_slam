...
class KeyframeManager():
    ...
    def read_kf_json(self, json_path):
        
        info = read_json_to_dict(json_path)
    ...
    def run(self):
        ...
        while True:
            # read keyframes
            json_file = f'{self.time_stamp.value}.json'
            json_path = os.path.join(self.input_folder, json_file)
            if os.path.exists(json_path):
                LBA_flag, GBA_flag, Loop_flag, optimized_kf, new_kf = self.read_kf_json(json_path)
                ...
                if self.init:
                    ...
                    self.new_volume_flag.value = True
                ...
                if LBA_flag or GBA_flag: 
                    ...
                    if not self.in_loop:
                        ...
                        if len(optimized_kf) > 0:
                            ...
                            if new_map == 3 and self.keyframe_list[-1] - anchor_frame >= self.map_length:
                                new_ref_kf = optimized_kf[0]
                                if self.verbose:
                ...
                if new_volume_flag:
                    ...
                    if len(new_kf) > 0:
                        self.local_keyframe_list[anchor_frame] = manager.list(new_kf)
                        self.local_optimized_keyframes[:] = manager.list(new_kf)
                    else:
                        self.local_keyframe_list[anchor_frame] = manager.list([anchor_frame])
                        self.local_optimized_keyframes[:] = manager.list([anchor_frame])
                    new_volume_flag = False
                    self.new_volume_flag.value = False
            ...
            if self.time_stamp.value >= self.n_img - 1:
                self.final.value = True
                while True:
                    if not self.final.value:
                        break
                break
